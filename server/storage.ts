...
export class MemStorage implements IStorage {
  ...
  async getRecentCalculations(): Promise<Array<{ id: string; timestamp: Date; result: CalculationResult }>> {
    ...
    return Array.from(calculations.entries())
      .map(([id, data]) => ({
        id,
      ...
      .slice(0, 10); // Return last 10 calculations
  }
  ...
  // Added function getStorageData
  private getStorageData(): Map<string, { input: CalculationInput; result: CalculationResult; timestamp: Date }> {
    try {
      ...
      console.error('Error retrieving calculations from localStorage:', error);
    }
    return new Map();
  }
}
